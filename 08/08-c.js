...
function sendExt (response , text ,ext){
    console.log(ext)
    let obj = {
        "plain" : "text/plain",
        "jpeg" : 'image/jpeg',
...
function readImg(request , response){
    let index = request.url.split('/')[2];
    fs.readFile(index , function(err , data) {
        
        if (err) {
            console.log ('ERR' , err)
            
            sendExt(response , 'ERR NO supported ext or file' , "plain")
        ...
        else 
        {
            let ext = index.split(".")
            console.log(ext)
            sendExt(response , data , ext[1].toLowerCase())
